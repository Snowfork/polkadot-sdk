...
fn set_token_transfer_fees() {
	new_test_ext(true).execute_with(|| {
		...
		assert_ok!(EthereumSystem::set_token_transfer_fees(origin, 1, 1, eth(1), 1));
	});
}
...
fn set_token_transfer_fees_root_only() {
	new_test_ext(true).execute_with(|| {
		...
		assert_noop!(EthereumSystem::set_token_transfer_fees(origin, 1, 1, eth(1), 1), BadOrigin);
	});
}
...
fn set_token_transfer_fees_invalid() {
	new_test_ext(true).execute_with(|| {
		...
		assert_noop!(
			EthereumSystem::set_token_transfer_fees(origin, 0, 0, eth(0), 0),
