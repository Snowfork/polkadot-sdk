		...
		let res = loop {
			...
			let res = match current_strategy {
				...
				RecoveryStrategy::ChunksFromValidators(inner, _) =>
					inner.run(&mut self.state, &mut self.sender, &self.params).await,
				RecoveryStrategy::SystematicChunks(inner, _) =>
					inner.run(&mut self.state, &mut self.sender, &self.params).await,
			};
...
pub enum RecoveryStrategy {
	...
	SystematicChunks(FetchSystematicChunks, Box<RecoveryStrategy>),
}
...
impl RecoveryStrategy {
	...
	fn display_name(&self) -> Option<&'static str> {
		match self {
			...
			Self::SystematicChunks(_, _) => Some("Systematic chunks recovery"),
		}
	}
	...
	pub fn then_fetch_full_from_backers(self: Box<Self>, params: FetchFullParams) -> Box<Self> {
		match *self {
			...
			Self::SystematicChunks(task, next) => {
				let next = next.then_fetch_full_from_backers(params);
				Box::new(Self::SystematicChunks(task, next))
			},
		}
	}
	...
	pub fn then_fetch_systematic_chunks(
		self: Box<Self>,
		params: FetchSystematicChunksParams,
	) -> Box<Self> {
		match *self {
			Self::Nil => Box::new(Self::SystematicChunks(FetchSystematicChunks::new(params), self)),
			Self::ChunksFromValidators(task, next) => {
				let next = next.then_fetch_systematic_chunks(params);
				Box::new(Self::ChunksFromValidators(task, next))
			},
			Self::FullFromBackers(task, next) => {
				let next = next.then_fetch_systematic_chunks(params);
	...
	fn pop_first(self: Self) -> (Self, Self) {
		match self {
			...
			Self::SystematicChunks(inner, next) =>
				(Self::SystematicChunks(inner, Box::new(Self::Nil)), *next),
		}
	}
}
...
pub struct FetchSystematicChunksParams {
	...
	pub(crate) validators: BTreeSet<ValidatorIndex>,
	// channel to the erasure task handler.
	pub(crate) erasure_task_tx: futures::channel::mpsc::Sender<ErasureTask>,
}
...
impl FetchSystematicChunks {
	pub fn new(params: FetchSystematicChunksParams) -> Self {
		Self {
			// We maintain the copy of the systematic validators in the params field, so that we
			// can use them when doing the recovery.
			validators: params.validators.clone().into_iter().collect(),
			params,
		}
	}
	...
	async fn run<Sender: overseer::AvailabilityRecoverySenderTrait>(
		...
		if !common_params.bypass_availability_store {
			...
			for i in &local_chunk_indices {
				...
				if self.params.validators.contains(i) && !state.received_chunks.contains_key(i) {
					gum::debug!(
						target: LOG_TARGET,
						candidate_hash = ?common_params.candidate_hash,
						erasure_root = ?common_params.erasure_root,
		...
		let mut systematic_chunk_count = self
			...
			.count();
		let systematic_chunk_threshold = self.params.validators.len();
		...
		loop {
			...
			if systematic_chunk_count >= systematic_chunk_threshold {
			...
			let desired_requests_count =
				self.get_desired_request_count(systematic_chunk_count, systematic_chunk_threshold);
			...
			let (total_responses, error_count) = state
				.wait_for_chunks(
					common_params,
					&mut self.validators,
					|unrequested_validators, reqs, chunk_count, _params, error_count| {
						error_count != 0 ||
							chunk_count >= systematic_chunk_threshold ||
							Self::is_unavailable(
								unrequested_validators,
								reqs,
								chunk_count,
	...
	async fn attempt_systematic_recovery(
		...
		let available_data = polkadot_erasure_coding::reconstruct_from_systematic_v1(
			common_params.n_validators,
			state.received_chunks.iter().map(|(index, chunk)| {
				(
					&chunk.chunk[..],
					usize::try_from(index.0).expect(
						"polkadot does not run on platforms where u32 cannot be cast into a usize",
					),
				)
			}),
		);
